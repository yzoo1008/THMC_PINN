...
pde_ds = tf.data.Dataset.from_tensor_slices((X_pde, Y_pde, T_pde, P_pde, Q_pde)).shuffle(10000).batch(batch_size)
...
for epoch in range(EPOCHS):
    ...
    p_loss = 0.0
    pde_loss = 0.0
    for x_tr, y_tr, t_tr, p_tr, q_tr in train_ds:
        loss, grad, loss_p, loss_pde, dp_dx, dp_dt, d2p_dx2 = train_step(x_tr, t_tr, p_tr, q_tr)
        train_loss += loss.numpy()/n_tr
        p_loss += loss_p.numpy()/n_tr
        pde_loss += loss_pde.numpy()/n_tr

        # np_d2p_dx2 = d2p_dx2.numpy()
        # np_dp_dx = dp_dx.numpy()
        # np_dp_dt = dp_dt.numpy()
        # np_x_tr = x_tr.numpy()
        # np_p_tr = p_tr.numpy()
        # np_q_tr = q_tr.numpy()
        # np_loss_p = loss_p.numpy()
        # np_loss_pde = loss_pde.numpy()
        # np_loss = loss.numpy()

    for x_val, y_val, t_val, p_val, q_val in val_ds:
        loss = test_step(x_val, t_val, p_val)
        val_loss += loss.numpy()/n_val
