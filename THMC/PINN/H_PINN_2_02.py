...
def get_diff_manual(X, T):
    P = model(tf.stack([X, T], axis=1))
    P_rs = tf.cast(tf.reshape(P, [40, -1]), dtype=tf.float64)
    T_rs = tf.cast(tf.reshape(T, [40, -1]), dtype=tf.float64)
    Pt = P_rs[0:-40, :] - P_rs[40:, :]
    Tt = T_rs[0:-40, :] - T_rs[40:, :]
    dp_dt = P_rs - P_rs
    dp_dt[0:-40, :] = Pt / Tt
...
def train_step(X, T, P, X_pde, T_pde, Q_pde):
    ...
    with tf.GradientTape() as tape:
        ...
        d2p_dx2, dp_dx, dp_dt = get_diff_manual(X_pde, T_pde)
        # 2. Loss 계산
