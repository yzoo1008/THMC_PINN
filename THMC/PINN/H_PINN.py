...
class PhysicsInformedNN:
    ...
    def get_pde_err(self, x, y, t, q):
        ...
        nu = [k, 20 / (24 * 60 * 60) / 1000 * (mu * B), phi * Ct * mu]

        p = self.get_p(x, y, t)

        # p_t = tf.gradients(p, t)[0]
        n_grid = self.n_grid
        p_t = (p[0:-n_grid] - p[n_grid:]) / 600
        # p_x = tf.gradients(p, x)[0]
        # p_xx = tf.gradients(p_x, x)[0]
        # self.p_xx = p_xx
        p_rs = tf.reshape(p, [-1, n_grid])
        p_trans = tf.transpose(p_rs)
        p_trans_rs = tf.reshape(p, [-1])

        p_x = (p_trans_rs[0:-n_grid] - p_trans_rs[n_grid:]) / 10
        p_xx = (p_x[0:-n_grid] - p_x[n_grid:]) / 10

        p_xx_rs = tf.reshape(p_xx, [-1, n_grid])
        p_xx_trans = tf.transpose(p_xx_rs)
        p_xx_ = tf.reshape(p_xx_trans, [-1])
        ...
        if self.dim == 1:
            ...
            pde_err = nu[0] * p_xx_ + nu[1] * q[0:-2*n_grid] - nu[2] * p_t[0:-n_grid]
...
if __name__ == "__main__":
    ...
    X_train, P_train, Q_train = g.train_data(100, n_t)
    X_pde, P_pde, Q_pde = g.pde_data(np.arange(n_t))#[1, 2])
    ...
    ### PLOT
    for t_idx in [0, 5, 10, 15, 20, 25, 30, 40]:
