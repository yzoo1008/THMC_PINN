...
# from plotting import newfig, savefig #여기서 가져와지지않아 이 파일에 직접 삽입함.
...
if __name__ == "__main__":
    ...
    data = scipy.io.loadmat('./Data/Flow_Data_101_101.mat')
...
######################################### subplot 으로 그리기
    ...
    # ####### Row 0: u(t,x) ##################
    #
    # ax = plt.subplot(211) #그래프 상하좌우 간격인가. spacing??
    #
    # h = ax.imshow(U_pred.T, interpolation='nearest', cmap='rainbow',
    #               extent=[t.min(), t.max(), x.min(), x.max()],
    #               origin='lower', aspect='auto', vmin= 100, vmax=150)
    ...
    # ax.set_ylabel('$x$')
    # ax.legend(frameon=False, loc='best')
    # ax.set_title('$predict--u(t,x)$', fontsize=10)
    #
    #
    #     ####### Row 1: u(t,x) slices ##################
    #
    # ax = plt.subplot(212)  # 그래프 상하좌우 간격인가. spacing??
    #
