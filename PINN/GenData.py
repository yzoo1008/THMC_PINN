...
class GenData:
    ...
    def grid_data_for_specific_t(self, t):
        ...
        x = self.X[n_grid * t:n_grid * (t + 1), 0:3]
    ...
    def train_data(self, n_I, n_B):
        ...
        if n_I == 0:
            return x_B, p_B, q_B
        return np.vstack([x_I, x_B]), np.vstack([p_I, p_B]), np.vstack([q_I, q_B])

    def pde_data(self, t_list):
        X, P, Q = self.grid_data_for_specific_t(t_list[0])
        if len(t_list) <= 1:
            return X, P, Q
        for t in t_list:
            x, p, q = self.grid_data_for_specific_t(t)
            X = np.vstack([X, x])
            P = np.vstack([P, p])
            Q = np.vstack([Q, q])
        return X, P, Q
