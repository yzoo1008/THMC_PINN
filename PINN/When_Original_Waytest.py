...
# from plotting import newfig, savefig  이거 쓰려니까 계속 오류가 나서 그냥 이 기능을 파일로 가져왔다.
...
for n in range(1,2):
    ...
    #Loss 출력하는 함수.
        def callback(self, loss, loss_val, loss_u, loss_f):
            ...
            if self.n % 25 ==0:
                # print('Loss:', loss)
                # val_dict = {self.x_val_tf: self.x_val, self.t_val_tf: self.t_val,
                #         self.u_val_tf: self.u_val}
                # XT_val = self.XT_val
                # u_val = self.u_val
                # u_val_pred = self.predict2(XT_val)
                # loss_val = tf.reduce_mean(tf.square(u_val - u_val_pred))

                # print('It: %d, Loss: %e, Val_loss: %e' %(self.n, loss, loss_val))
                # print('It: %d, Loss: %e, Loss_val: %e' % (self.n, loss, loss_val))
    ...
    ######################################### N_u 최적화 하고 싶으면 이 값을 조정하면 된다.
        ...
        for i in range(len(N_u)):
            ...
            #
            # savefig('./figures/flow_equation_PDE')
        #
